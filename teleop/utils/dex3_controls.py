...
def trigger_to_dex3_targets(trigger: float, open_pose: np.ndarray, closed_pose: np.ndarray) -> np.ndarray:
    ...
    ``1.0`` when fully pressed. Invalid values fail open; finite values are
    clamped before interpolation.
    """
    open_pose = np.asarray(open_pose, dtype=float)
    closed_pose = np.asarray(closed_pose, dtype=float)
    if open_pose.shape != (7,) or closed_pose.shape != (7,):
    ...
    return open_pose + amount * (closed_pose - open_pose)
